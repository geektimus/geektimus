...
function generateStatsSvg(stats) {
  ...
  const rows = [
    ...
    {
      icon: 'M1.5 3.25a2.25 2.25 0 1 1 3 2.122v5.256a2.251 2.251 0 1 1-1.5 0V5.372A2.25 2.25 0 0 1 1.5 3.25Zm5.677-.177L9.5 5.396V2.75a.75.75 0 0 1 1.5 0v3.75a.75.75 0 0 1-.75.75H6.5a.75.75 0 0 1 0-1.5h1.94L6.116 3.371a.75.75 0 0 1 1.061-1.061Z',
      label: 'Total PRs:',
      value: `${stats.totalPRs}`
    },
    {
      icon: 'M2 2.5A2.5 2.5 0 0 1 4.5 0h8.75a.75.75 0 0 1 .75.75v12.5a.75.75 0 0 1-.75.75h-2.5a.75.75 0 0 1 0-1.5h1.75v-2h-8a1 1 0 0 0-.714 1.7.75.75 0 1 1-1.072 1.05A2.495 2.495 0 0 1 2 11.5Zm10.5-1h-8a1 1 0 0 0-1 1v6.708A2.486 2.486 0 0 1 4.5 9h8ZM5 12.25a.25.25 0 0 1 .25-.25h6.5a.25.25 0 0 1 .25.25v1a.25.25 0 0 1-.25.25h-6.5a.25.25 0 0 1-.25-.25Z',
      label: 'Public Repos:',
      value: `${stats.publicRepos}`
    },
    {
      icon: 'M2 5.5a3.5 3.5 0 1 1 5.898 2.549 5.508 5.508 0 0 1 3.034 4.084.75.75 0 1 1-1.482.235 4 4 0 0 0-7.9 0 .75.75 0 0 1-1.482-.236A5.507 5.507 0 0 1 3.102 8.05 3.493 3.493 0 0 1 2 5.5ZM11 4a3.001 3.001 0 0 1 2.22 5.018 5.01 5.01 0 0 1 2.56 3.66.75.75 0 0 1-1.46.344 3.512 3.512 0 0 0-2.027-2.87 2.998 2.998 0 0 1-3.664-3.084C9.18 6.42 10.02 5.5 11 4Z',
      label: 'Contributed to:',
      value: `${stats.totalForks + 10}+`
    }
  ];
  ...
  const renderedRows = rows.map((row, i) => {
    ...
    return `
    <g>
      <svg class="icon" viewBox="0 0 16 16" width="16" height="16" x="25" y="${y - 12}">
        <path d="${row.icon}"></path>
      </svg>
      <text class="stat-label" x="50" y="${y}">${row.label}</text>
      <text class="stat-value" x="210" y="${y}">${row.value}</text>
    </g>`;
  }).join('');
  ...
  <style>
    ...
    .rank-circle { fill: none; stroke: ${THEME.accentCyan}; stroke-width: 4; stroke-dasharray: 250; stroke-dashoffset: 25; }
  ...
  <!-- Title -->
  <text class="header" x="25" y="32">Alex Cano's GitHub Stats</text>

  <!-- Divider line -->
  <line x1="25" y1="46" x2="${width - 25}" y2="46" stroke="#3e3352" stroke-width="1" />

  <!-- Stats Rows -->
  ${renderedRows}

  <!-- Rank Badge -->
  <g transform="translate(390, 116)">
    <circle cx="0" cy="0" r="42" fill="#20172c" />
    <circle cx="0" cy="0" r="42" class="rank-circle" />
    <text text-anchor="middle" y="8" class="rank-text">A+</text>
